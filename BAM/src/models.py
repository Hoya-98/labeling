...
class ResNetV2(nn.Module):
  ...
  def forward(self, x):
    ...
    # print("Size of x right after passing block1 : ", x1.size())

    # if not self.bam1 is None:
    #     x = self.bam1(x)

    # print("Size of x right after passing bam1 : ", x1.size())

    x = self.body.block2(x)

    # print("Size of x right after passing block2 : ", x2.size())

    # if not self.bam2 is None:
    #     x = self.bam2(x)

    # print("Size of x right after passing bam2 : ", x2.size())
    ...
    x4 = self.body.block4(x)

    x = self.head(x4)   
    
    # print(x1[...,0,0].size())
    assert x.shape[-2:] == (1, 1)  # We should have no spatial shape left.
    return x[...,0,0], torch.flatten(x4, start_dim=1)
